...
export async function GET(request: Request) {
  ...
  try {
    const result = await session.run(`MATCH (n:Entity)
      ...
      RETURN collect(DISTINCT {id:n.id, name:n.name, type:n.type, alias:n.alias, jurisdiction:n.jurisdiction})[0..$limit] AS nodes,
        collect(DISTINCT {id:coalesce(r.id, n.id + "-" + m.id), source:n.id, target:m.id, type:coalesce(r.type, type(r)), status:coalesce(r.status, "observed"), sources:coalesce(r.sources, [r.source])})[0..$limit] AS edges`, { limit: neo4j.int(limit), caseId })
