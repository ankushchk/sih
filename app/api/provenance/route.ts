...
import { buildLiveProofRun } from '@/lib/liveProvenance'
...
export async function GET(request: Request) {
  ...
  const liveRun = await buildLiveProofRun(caseId)
  const run = liveRun.resourceCount > 0 ? liveRun : buildProofRun(caseId)
  try {
...
export async function POST(request: Request) {
  ...
  const caseId = body?.caseId || 'CASE-1004'
  const liveRun = await buildLiveProofRun(caseId)
  const run = liveRun.resourceCount > 0 ? liveRun : buildProofRun(caseId)
