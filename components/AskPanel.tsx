...
export function AskPanel({ onCiteClick, onEntityClick }: { onCiteClick?: (sourceId: string) => void; onEntityClick?: (id: string) => void }) {
  ...
  async function handleAsk() {
    ...
    setError(null)
    try {
      const response = await fetch('/api/ask', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question, caseId: activeCaseId }) })
  ...
  return <section className="panel graph-rag-panel">
    <div className="panel-heading"><div><span className="eyebrow">GRAPH RAG / {activeCaseId}</span><h2>Ask the evidence graph</h2></div><span className="status-pill corroborated"><i /> Grounded retrieval</span></div>
    ...
    <div className="graph-rag-input"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && handleAsk()} placeholder={`Ask about ${activeCaseId}...`} /><button className="primary-button small" onClick={handleAsk} disabled={loading}>{loading ? 'Retrieving...' : 'Ask'}</button></div>
