...
const cases = [
  ['CASE-1001', 'Warehouse 7 meeting correlation', 'ARCHIVED', 'LOW', 'Noida'],
  ['CASE-1002', 'Blue Arc Office association review', 'REVIEW', 'MEDIUM', 'Noida'],
  ['CASE-1003', 'Old Industrial Road financial trail', 'ACTIVE', 'MEDIUM', 'Ghaziabad'],
  ['CASE-1004', 'Cafe Meridian Network Demonstration', 'ACTIVE', 'HIGH', 'Delhi / Noida'],
  ['CASE-1005', 'Sector 18 convergence review', 'ACTIVE', 'MEDIUM', 'Noida'],
]
...
export async function seed() {
  ...
  try {
    ...
    await session.run('UNWIND $rows AS row MERGE (n:Case {id: row[0]}) SET n.title = row[1], n.status = row[2], n.priority = row[3], n.jurisdiction = row[4]', { rows: cases })
